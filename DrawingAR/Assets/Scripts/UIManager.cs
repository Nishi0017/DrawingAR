...
public class UIManager : MonoBehaviour
{
    ...
    private void AnimationUI(bool toActive)
    {
        ...
        if (toActive)
        {
            ...
            uiRectTransforms[nowActiveNum].position = leftHand.transform.position;
        ...
        else
        {
            ...
            // ローカル座標のアニメーション
            uiRectTransforms[nowActiveNum].DOMove(leftHand.transform.position, 0.5f)
                .SetEase(Ease.InSine); // イージング関数を設定
    ...
    private void SwitchUI(int num)
    {
        ...
        // ローカル座標のアニメーション
        uiRectTransforms[num].DOMove(leftHand.transform.position, 0.5f)
            .SetEase(Ease.InSine); // イージング関数を設定
